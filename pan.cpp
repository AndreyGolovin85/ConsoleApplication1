...
Pan::Pan()
{
    Speed_Pan = 250;     // Переменная скорости сковороды.
    Pan_Position.x = 400;// Переменная x  сковороды.
    ...
    Pan_Image.createMaskFromColor(Pan_Image.getPixel(0, 0));       // Удаляем белый фон у картинки
    ...
    void Pan::update(float elapsedTime)
    {
        ...
        else if (RightPressed)                                                        // Правого края экрана, если да то сковорда останавливается.
        {
